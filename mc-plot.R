...
for(o in outcome.vars){
  for(c in covarflag){
    ...
    mc.estimates.cbw.eastern <- readRDS(paste0("results/mc-estimates-cbw-eastern-",o,c,".rds"))
    boot.cbw.eastern <- readRDS(paste0("results/boot-cbw-eastern-",o,c,".rds"))
    ...
    ggsave(paste0("plots/mc-estimates-cbw-eastern-",o,c,".png"), mc.plot, width=8.5, height=11)
    
    # Swiss cluster
    
    outcomes.cbw.swiss <- readRDS(paste0("data/outcomes-cbw-swiss-",o,".rds"))
    mc.estimates.cbw.swiss <- readRDS(paste0("results/mc-estimates-cbw-swiss-",o,c,".rds"))
    boot.cbw.swiss <- readRDS(paste0("results/boot-cbw-swiss-",o,c,".rds"))
    
    mc.plot <- PlotMCCapacity(observed = outcomes.cbw.swiss$M, 
                              y.title=outcomes.labels[which(outcome.vars==o)],
                              main = "Retrospective prediction for later-treated in Swiss cluster",
    ...
    ggsave(paste0("plots/mc-estimates-cbw-swiss-",o,c,".png"), mc.plot, width=8.5, height=11)
    ...
    mc.estimates.lm.eastern <- readRDS(paste0("results/mc-estimates-lm-eastern-",o,c,".rds"))
    boot.lm.eastern <- readRDS(paste0("results/boot-lm-eastern-",o,c,".rds"))
    ...
    ggsave(paste0("plots/mc-estimates-lm-eastern-",o,c,".png"), mc.plot, width=8.5, height=11)
    
    # Swiss cluster
    
    outcomes.lm.swiss <- readRDS(paste0("data/outcomes-lm-swiss-",o,".rds"))
    mc.estimates.lm.swiss <- readRDS(paste0("results/mc-estimates-lm-swiss-",o,c,".rds"))
    boot.lm.swiss <- readRDS(paste0("results/boot-lm-swiss-",o,c,".rds"))
    
    mc.plot <- PlotMCCapacity(observed = outcomes.lm.swiss$M, 
                              y.title=outcomes.labels[which(outcome.vars==o)],
                              main = "Prospective prediction for later-treated in Swiss cluster",
    ...
    ggsave(paste0("plots/mc-estimates-lm-swiss-",o,c,".png"), mc.plot, width=8.5, height=11)
  }
}
...
for(c in covarflag){
  ...
  iid <- lapply(outcome.vars, function(o){
    p <- readRDS(paste0("results/iid-cbw-eastern-",o,c,".rds"))
    return(p)
  })
  names(iid) <- outcome.vars
  
  iid.block <- lapply(outcome.vars, function(o){
    p <- readRDS(paste0("results/iid-block-cbw-eastern-",o,c,".rds"))
    return(p)
  })
  names(iid.block) <- outcome.vars
  
  moving.block <- lapply(outcome.vars, function(o){
    p <- readRDS(paste0("results/moving-block-cbw-eastern-",o,c,".rds"))
    return(p)
  })
  ...
  iid <- lapply(outcome.vars, function(o){
    p <- readRDS(paste0("results/iid-cbw-swiss-",o,c,".rds"))
    return(p)
  })
  names(iid) <- outcome.vars
  
  iid.block <- lapply(outcome.vars, function(o){
    p <- readRDS(paste0("results/iid-block-cbw-swiss-",o,c,".rds"))
    return(p)
  })
  names(iid.block) <- outcome.vars
  
  moving.block <- lapply(outcome.vars, function(o){
    p <- readRDS(paste0("results/moving-block-cbw-swiss-",o,c,".rds"))
    return(p)
  })
  names(moving.block) <- outcome.vars
  ...
  iid <- lapply(outcome.vars, function(o){
    p <- readRDS(paste0("results/iid-lm-eastern-",o,c,".rds"))
    return(p)
  })
  names(iid) <- outcome.vars
  
  iid.block <- lapply(outcome.vars, function(o){
    p <- readRDS(paste0("results/iid-block-lm-eastern-",o,c,".rds"))
    return(p)
  })
  names(iid.block) <- outcome.vars
  
  moving.block <- lapply(outcome.vars, function(o){
    p <- readRDS(paste0("results/moving-block-lm-eastern-",o,c,".rds"))
    return(p)
  })
  names(moving.block) <- outcome.vars
  ...
  iid <- lapply(outcome.vars, function(o){
    p <- readRDS(paste0("results/iid-lm-swiss-",o,c,".rds"))
    return(p)
  })
  names(iid) <- outcome.vars
  
  iid.block <- lapply(outcome.vars, function(o){
    p <- readRDS(paste0("results/iid-block-lm-swiss-",o,c,".rds"))
    return(p)
  })
  names(iid.block) <- outcome.vars
  
  moving.block <- lapply(outcome.vars, function(o){
    p <- readRDS(paste0("results/moving-block-lm-swiss-",o,c,".rds"))
    return(p)
  })
  names(moving.block) <- outcome.vars
